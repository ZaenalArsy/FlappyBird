...
extension GameScene {
    ...
    func createBird() -> SKSpriteNode {
        ...
        //1
        let bird = SKSpriteNode(texture: SKTextureAtlas(named: "bird").textureNamed("bird-01"))
        bird.size = CGSize(width: 50, height: 50)
        bird.position = CGPoint(x: self.frame.midX, y: self.frame.midY)
        
        //2
        bird.physicsBody = SKPhysicsBody(circleOfRadius: bird.size.width / 2)
        bird.physicsBody?.linearDamping = 1.1 //lambat jatuhnya, semakin besar semakin lambat (air friction)
        ...
        //4
        bird.physicsBody?.affectedByGravity = false //disini kecepatan gravitynya tidak diatur, dan menggunakan defaultnya. contohnya : physicsWorld.gravity = CGVector(dx: 0.0, dy:  -0.1)
    ...
    func createHighscoreLabel() -> SKLabelNode {
        ...
        if let highestScore = UserDefaults.standard.object(forKey: "highestScore"){ //mengambil score dari database. The highscore is saved in UserDefaults, if the value is not found in the UserDefaults (for example when app is first launched), then the label text will be set to zero.
    ...
    func createLogo() {
        ...
        logoImg = SKSpriteNode(imageNamed: "logoZ")
        //logoImg.size = CGSize(width: 272, height: 65)
        logoImg.size = CGSize(width: 180, height: 110)
        logoImg.position = CGPoint(x: self.frame.midX, y: self.frame.midY + 120)
